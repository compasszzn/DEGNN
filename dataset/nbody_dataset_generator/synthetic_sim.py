...
class SpringSim(object):
    def __init__(self, n_balls=5, box_size=5., loc_std=.5, vel_norm=.5,
        ...
        self.box_size_new = np.array([2.5, 2, 1.5])
    ...
    def _clamp_new(self, loc, vel):
        '''
        :param loc: 3xN location at one time stamp
        :param vel: 3xN velocity at one time stamp
        :return: location and velocity after hiting walls and returning after
            elastically colliding with walls
        '''
        box_size = self.box_size_new.reshape(-1,1)
        assert (np.all(loc < box_size * 3))
        assert (np.all(loc > -box_size * 3))

        over = loc > box_size
        loc[over] = np.tile(2 * box_size,loc.shape[-1])[over]  - loc[over]
        ...
        under = loc < -box_size
        loc[under] = np.tile(-2 * box_size,loc.shape[-1])[under] - loc[under]
        # assert (np.all(vel[under] < 0))
        assert (np.all(loc >= - box_size))
        vel[under] = np.abs(vel[under])
        #print(box_size)
    ...
    def sample_trajectory(self, T=10000, sample_freq=10,
        ...
        loc[0, :, :], vel[0, :, :] = self._clamp_new(loc_next, vel_next)
        # disables division by zero warning, since I fix it with fill_diagonal
        with np.errstate(divide='ignore'):
            ...
            for i in range(1, T):
                ...
                # loc_next, vel_next = self._clamp_new(loc_next, vel_next)
                loc_next, vel_next = self._clamp_new(loc_next, vel_next)
...
class ChargedParticlesSim(object):
    def __init__(self, n_balls=5, box_size=5., loc_std=1., vel_norm=0.5,
        ...
        self.n_balls = n_balls
        self.box_size = box_size
        self.box_size_new = np.array([20, 16, 12])
        self.loc_std = loc_std
    ...
    def sample_trajectory(self, T=10000, sample_freq=10,
        ...
        # print("box_size_new",self.box_size_new)
        n = self.n_balls
        assert (T % sample_freq == 0)
        T_save = int(T / sample_freq - 1)
        diag_mask = np.ones((n, n), dtype=bool)
...
class GravitySim(object):
    def __init__(self, n_balls=100, loc_std=1, vel_norm=0.5, interaction_strength=1, noise_var=0, dt=0.001, softening=0.1):
        ...
        self.interaction_strength = interaction_strength
        self.noise_var = noise_var
        self.box_size_new = np.array([5, 4, 4])
        self.dt = dt
    ...
    def _clamp_new(self, loc, vel):
        ...
        :return: location and velocity after hiting walls and returning after
            elastically colliding with walls
        '''
        box_size = self.box_size_new
        assert (np.all(loc < box_size * 3))
        assert (np.all(loc > -box_size * 3))

        over = loc > box_size
        loc[over] = np.tile(2 * box_size,(loc.shape[0],1))[over]  - loc[over]
        assert (np.all(loc <= box_size))
        ...
        under = loc < -box_size
        loc[under] = np.tile(-2 * box_size,(loc.shape[0],1))[under]  - loc[under]
        # assert (np.all(vel[under] < 0))
        assert (np.all(loc >= - box_size))
        vel[under] = np.abs(vel[under])
        #print(box_size)
    ...
    def sample_trajectory(self, T=10000, sample_freq=10):
        ...
        pos, vel = self._clamp_new(pos, vel)
        # pos, vel = self._clamp(pos, vel)
...
if __name__ == '__main__':
    ...
    sim = ChargedParticlesSim(n_balls=100, loc_std=1)
