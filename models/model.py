...
from models.models.gcl import E_GCL_vel,DE_GCL_vel_2D
import numpy as np
...
class DEGNN_2D(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf,model_type,pool_method, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0,
                 recurrent=False, norm_diff=False, tanh=False):
        super(DEGNN_2D, self).__init__()
        self.hidden_nf = hidden_nf
        self.device = device
        self.n_layers = n_layers
        self.embedding = nn.Linear(in_node_nf, self.hidden_nf)
        self.module = DE_GCL_vel_2D(self.hidden_nf, self.hidden_nf, self.hidden_nf,model_type,pool_method, edges_in_d=in_edge_nf, device=device, 
                                                    act_fn=act_fn, coords_weight=coords_weight, recurrent=recurrent,
                                                    norm_diff=norm_diff, tanh=tanh)
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, DE_GCL_vel_2D(self.hidden_nf, self.hidden_nf, self.hidden_nf,model_type,pool_method, edges_in_d=in_edge_nf,
                                                    act_fn=act_fn, coords_weight=coords_weight, recurrent=recurrent,
                                                    norm_diff=norm_diff, tanh=tanh))
